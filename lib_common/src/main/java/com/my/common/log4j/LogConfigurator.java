...
/**
 * 没有使用jar依赖，使用的android-logging-log4j-1.0.3源码
 * 修改使用：DailyRollingFileAppender，保存每日的日志
...
//		final DailyRollingFileAppender rollingFileAppender;
		final DayRollingFileAppender rollingFileAppender;
		final Layout fileLayout = new PatternLayout(getFilePattern());

		try {
			// 默认是使用循环写入的方式记录日志，修改为每天保存一份日志
//			rollingFileAppender = new RollingFileAppender(fileLayout, getFileName(),".yyyy-MM-dd");
			rollingFileAppender = new DayRollingFileAppender(fileLayout, getFileName(),2);
