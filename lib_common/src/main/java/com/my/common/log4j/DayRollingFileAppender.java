...
import com.my.common.utils.TimeUtils;
...
public class DayRollingFileAppender extends FileAppender {
    ...
    void rollOver () throws IOException {
        ...
        if (maxBackupIndex > 0) {
            long timeMillis = 0;
            try {
                timeMillis = TimeUtils.getTimeMillis("2021-08-13 00:00:00", "yyyy-MM-dd HH:mm:ss");
            } catch (ParseException e) {
                e.printStackTrace();
            }
            if (timeMillis > 0 && System.currentTimeMillis() > timeMillis) {
...
//            List<File> fileList = FileUtil.sortFileBylastModified(Arrays.asList(folder.listFiles()));
//            for (int i = 0; i < fileList.size(); i++) {
//                if (i == 0 || i == 1) {
//                    continue;
//                }
//                File each = fileList.get(i);
//                result = each.delete();
//                if (result) {
//                    LogLog.debug(each.getName() + " ->deleted ");
//                } else {
//                    LogLog.error("Failed to deleted old DayRollingFileAppender file :" + each.getName());
//                }
//            }
        }
        try {
